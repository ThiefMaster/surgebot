	...
	if(node->type != DB_EMPTY)
	{
		switch(node->type)
		{
			...
			case DB_STRINGLIST:
				...
				break;
			default:
				log_append(LOG_ERROR, "Invalid node type: %d", node->type);
		}
	}
...
int database_read(struct database *db, unsigned int free_nodes_after_read)
{
	...
	{
		log_append(LOG_WARNING, "Could not open database %s (%s) for reading: %s (%d)", db->name, db->filename, strerror(errno), errno);
		return -1;
	}

	if(fstat(fileno(db->fp), &statinfo))
	{
		log_append(LOG_ERROR, "Could not fstat database file %s (database %s): %s (%d)", db->filename, db->name, strerror(errno), errno);
		return -2;
	}
	...
	{
		log_append(LOG_ERROR, "mmap() failed: %s (%d), falling back to file mode", strerror(errno), errno);
		db->source = SRC_FILE;
		db->map = NULL;
	}
#else
	...
	{
		unsigned int i;
		log_append(LOG_ERROR, "Parse error in database %s on line %d at position %d: %s", db->name, db->line, db->line_pos, errors[result]);
	...
	switch(db->source)
	{
		...
		case SRC_MMAP:
			...
			break;
		default:
			log_append(LOG_ERROR, "Invalid database source in database_read(): %d", db->source);
			return EOF;
	}
...
static unsigned int database_eof(struct database *db)
{
	switch(db->source)
	{
		...
		case SRC_MMAP:
			...
			break;
		default:
			log_append(LOG_ERROR, "Invalid database source in database_eof(): %d", db->source);
			return 1;
			break;
	}
}
...
static char database_getc(struct database *db)
{
	...
	switch(db->source)
	{
		...
		case SRC_MMAP:
			...
			break;
		default:
			log_append(LOG_ERROR, "Invalid database source in database_getc(): %d", db->source);
			return EOF;
	}
...
static void database_ungetc(struct database *db, char c)
{
	switch(db->source)
	{
		...
		case SRC_MMAP:
			...
			break;
		default:
			log_append(LOG_ERROR, "Invalid database source in database_ungetc(): %d", db->source);
	}
...
int database_write(struct database *db)
{
	...
	{
		log_append(LOG_WARNING, "Could not open database %s (%s) for writing: %s (%d)", db->name, db->tmp_filename, strerror(errno), errno);
		return -1;
	}
	...
	{
		log_append(LOG_ERROR, "Writing %s failed, %d unclosed objects", db->name, db->indent);
		fclose(db->fp);
	...
	{
		log_append(LOG_WARNING, "Writing %s failed, return code was %d", db->name, result);
		fclose(db->fp);
		remove(db->tmp_filename); // delete temp. database file
		return result;
	}
	else // write func returned success
	{
		...
		db->last_write = now;
		log_append(LOG_INFO, "Database %s successfully written", db->name);
		return 0;
	}
}
...
void database_fini()
{
	assert(dict_size(databases) == 0); // all modules should delete their databases on unload
	dict_free(databases);
}
