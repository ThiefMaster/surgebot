...
MODULE_INIT
{
	...
	dict_set_free_funcs(chanlogs, free, (dict_free_f*)chanlog_free);
	
	DEFINE_COMMAND(self, "stats chanlog",		stats_chanlog,		1, 0, "group(admins)");
...
static void chanlog(const char *target, const char *format, ...)
{
	...
	if(fprintf(fd, "%s %s\n", timestamp, strip_codes(buf)) < 0)
	{
		log_append(LOG_ERROR, "Failed writing to logfile.");
		return;
	}
	fflush(fd);
}
...
COMMAND(stats_chanlog)
{
	...
	table_set_header(table, "Channel", "PurgeAfter");
	...
	dict_iter(node, chanregs)
	{
		...
		table->data[i][1] = chanreg_setting_get(reg, cmod, "PurgeAfter");
		i++;
	}
