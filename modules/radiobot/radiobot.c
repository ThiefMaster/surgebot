...
MODULE_DEPENDS("commands", "help", "http", "httpd", "sharedmem", "tools", "radioapi", NULL);
...
IRC_HANDLER(nick)
{
	...
	if(current_mod && !strcmp(current_mod, src->nick))
	{
		...
		radioapi_call("nickchange", json_object_to_json_string(payload));
...
COMMAND(setmod)
{
	...
	if(mod_changed)
	{
		...
		radioapi_call("setmod", json_object_to_json_string(payload));
		json_object_put(payload);
	}
