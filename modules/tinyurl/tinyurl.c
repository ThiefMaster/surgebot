...
static struct tinyurl *tinyurl_create(char *channel, const char *alias);
...
IRC_HANDLER(privmsg)
{
	...
	// Do we actually have to parse this?
	if(!(tmp = strcasestr(argv[2], "tinyurl.com")))
		return;
	...
	tinyurl = tinyurl_create(argv[1], alias);
	free(alias);
}
...
static struct tinyurl *tinyurl_create(char *channel, const char *alias)
{
	...
	sprintf(request, "tinyurl.com/%s", alias);
	tinyurl->http = HTTPRequest_create(request, event_func, read_func);
	tinyurl->http->forward_request = 0;
